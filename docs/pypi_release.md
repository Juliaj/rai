...
2. Build wheels and source distributions for all packages (including `rai-pubtest`)
3. Publish `rai-pubtest` to Test PyPI first (validates Trusted Publisher authentication)
4. Publish other packages to Test PyPI and verify installation
5. Manual approval (GitHub Environment: `pypi`)
6. Publish `rai-pubtest` to PyPI first (validates Trusted Publisher authentication)
...
In dry-run mode, the workflow skips publishing to Test PyPI and PyPI. It builds all distributions (including `rai-pubtest`), installs from locally built wheels, and runs import verification tests. Use `dry_run=true` to validate workflow changes without consuming Test PyPI/PyPI versions.

Notes:

-   `rai-pubtest` is a minimal test package used to validate the Trusted Publisher authentication path before publishing real packages. It is published to both Test PyPI and production PyPI as a canary to catch authentication issues early.
